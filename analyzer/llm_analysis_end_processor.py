...
class LLMAnalysisEndProcessor:
    ...
    def _run_async_wrapper(self, report_ids_str_list):
        ...
        try:
            ...
            # Create a new event loop for this thread if one doesn't exist, or use existing one.
        ...
        if report_ids:
            ...
            if not valid_ids_to_process:
                ...
                for pid in valid_ids_to_process:
                    try:
                        await self._process_seo_report(pid)
            ...
            self.logger.info(f"LLMAnalysisEndProcessor.run called with no specific actions requested (no report_ids, process_pending=False).")
...
if __name__ == '__main__':
    async def main_test_runner():
        ...
        try:
            ...
            await processor.run(process_pending=True, batch_size=2) # Small batch for testing

            # Example: Test scheduling in background
            # Ensure main_test_runner waits if you want to see the thread complete
            # processor.schedule_run_in_background(report_ids=[280]) # Replace with a valid ID
            # logger.info("Scheduled background processing. Main test might exit before completion.")
            # await asyncio.sleep(20) # Give some time for thread to work if testing this
