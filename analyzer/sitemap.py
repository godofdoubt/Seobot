...
async def _parse_sitemap_xml_content(content: str, sitemap_url_source: str) -> Tuple[List[str], List[str]]:
    ...
    try:
        ...
        if "sitemapindex" in root.tag.lower():
            ...
            # Fallback for sitemaps that might not strictly adhere to urlset/sitemapindex as root
            ...
            for loc_elem in root.findall(f".//{namespace}loc"):
                if loc_elem is not None and loc_elem.text:
                    ...
                    # Heuristic: if it ends with .xml, assume it's a sitemap link, else a page URL
        ...
        logger.error(f"XML parsing error in sitemap {sitemap_url_source}: {e}. Content: {content[:500]}...") # Log part of content
        return [], [] # Return empty lists on parsing error
    except Exception as e:
        logger.error(f"Unexpected error parsing sitemap content from {sitemap_url_source}: {e}")
        return [], []

async def _fetch_and_parse_single_sitemap(sitemap_url: str, session: aiohttp.ClientSession, visited_sitemaps: Set[str]) -> Set[str]:
    """
    ...
    try:
        async with session.get(sitemap_url, timeout=20) as response: # Increased timeout for sitemaps
            if response.status == 200:
                ...
                for child_s_url_relative in child_sitemap_urls:
                    # Ensure child sitemap URLs are absolute
                    absolute_child_s_url = urljoin(sitemap_url, child_s_url_relative)
                    if absolute_child_s_url not in visited_sitemaps:
                        recursive_page_urls = await _fetch_and_parse_single_sitemap(absolute_child_s_url, session, visited_sitemaps)
...
async def discover_sitemap_urls(domain_url: str, session: aiohttp.ClientSession) -> List[str]:
    ...
    # 2. Check common locations (add more if needed)
    #    These are checked even if robots.txt provides some, as a fallback or supplement.
    common_paths = ["/sitemap.xml", "/sitemap_index.xml", "/sitemap.xml.gz", "/sitemap-index.xml.gz"] # .gz would need gzip decompression
    
    # Create absolute URLs for common paths and add them if not already discovered
    # Basic check, actual existence will be verified during fetching.
...
async def fetch_all_pages_from_sitemaps(discovered_sitemap_urls: List[str], session: aiohttp.ClientSession) -> Set[str]:
    ...
    visited_sitemaps_cache: Set[str] = set() # Cache to avoid re-processing same sitemap file

    # Before fetching, try a HEAD request to see if the sitemap likely exists
    # This helps avoid 404s for common paths that don't exist.
    valid_sitemap_urls_to_fetch: List[str] = []
    for s_url in discovered_sitemap_urls:
        try:
            async with session.head(s_url, timeout=5, allow_redirects=True) as resp: # Allow redirects for HEAD
                if resp.status == 200:
                    valid_sitemap_urls_to_fetch.append(s_url)
        ...
        except Exception as e: # Catch any other exception during HEAD
            logger.error(f"Unexpected error during HEAD request for {s_url}: {e}")


    for s_url in valid_sitemap_urls_to_fetch:
        if s_url not in visited_sitemaps_cache: # Check cache before starting fetch for a root sitemap URL
            page_urls_from_sitemap = await _fetch_and_parse_single_sitemap(s_url, session, visited_sitemaps_cache)
