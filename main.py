...
# --- Configuration & Setup --- (Keep these if they don't involve Streamlit elements directly)
load_dotenv()
logging.basicConfig(
    level=logging.INFO,
...
# This is fine here as it's session state manipulation, not a Streamlit command that must be first
...
# Initialize Supabase client (can be here)
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)


# --- Functions that can be imported by other pages ---
...
async def process_url(url, lang="en"):
    # ... (keep your existing process_url function code here)
    # Ensure this function does not call st.set_page_config()
    try:
        # Normalize URL
...
# --- Main application logic for main.py page ---
def run_main_app():
    # This MUST be the first Streamlit command for the main page
    st.set_page_config(
    ...
    if not st.session_state.authenticated:
        ...
        if st.session_state.analysis_complete and hasattr(st.session_state, 'text_report') and st.session_state.text_report and hasattr(st.session_state, 'url') and st.session_state.url:
            ...
            with col1:
                if st.button(language_manager.get_text("article_writer_button", lang), key="article_writer_button_results", use_container_width=True):
                    # Assuming update_page_history is available or defined elsewhere if needed before switch
        ...
        if st.sidebar.button(language_manager.get_text("logout_button", lang), key="logout_button"):
            ...
            st.session_state.analysis_complete = False
            st.session_state.detailed_analysis_info = {"report_id": None, "url": None, "status_message": ""}
            for key_to_del in ['text_report', 'full_report', 'url']:
...
if __name__ == "__main__":
    run_main_app() # This will now include the st.set_page_config call